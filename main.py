...
from src import shuffle, spotify_custom

parser = argparse.ArgumentParser(
    description="Shuffle spotify playlist",
)
parser.add_argument("playlist_id", type=str, help="Playlist id")
parser.add_argument(
...
async def main(
    ...
    *,
    ...
    def get_recent_tracks_in_playlist(
        ...
        recent_tracks = [
            d["track"] for d in recent_track_dicts if d["context"].uri == playlist_uri
        ]

        # Filter tracks that are in playlist now
        all_playlist_uris = [track.uri for track in all_playlist_tracks]
        recent_tracks = [
            track for track in recent_tracks if track.uri in all_playlist_uris
        ]
    ...
    def early_shuffling_check(
        ...
        recent_tracks_idx = [
            all_playlist_urls.index(track.uri) for track in recent_tracks
        ]
        # Checking monotonicity
        # order is from most recent to the least recent
        condition = all(
            i >= j for i, j in zip(recent_tracks_idx, recent_tracks_idx[1:])
        )
    ...
    all_playlist_tracks = await spotify_custom.get_all_tracks_from_playlist(
        playlist_id, client
    )
